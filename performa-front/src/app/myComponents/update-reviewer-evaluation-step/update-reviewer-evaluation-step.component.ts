...
@Component({
  selector: 'app-update-reviewer-evaluation-step',
  templateUrl: './update-reviewer-evaluation-step.component.html',
  styleUrl: './update-reviewer-evaluation-step.component.scss'
})
export class UpdateReviewerEvaluationStepComponent {
  ...
  updateEvaluationModalVisibility = false
  ...
  updateAndSubmitEvaluationData() {
    ...
    for (const key in this.evaluationToEvaluate.evaluationCompetencies) {
      ...
      if (item.appraiser_mark ==0 || item.weightage==0) {
        isEmptyOrNull = true;
        break;
      }
    }
  ...
  updateEvaluation() {
    this.evaluationService.updateEvaluationReviewerStep(this.evaluationToEvaluate).subscribe({
      next: EvaluationData => {
  ...
  generateExcelFile(id: number | null) {
    ...
    ///////////////////////////// Modal Confirm update evaluation functions ///////////////////////////////////

    handleUpdateEvalutionModal(event: any) {
      this.updateEvaluationModalVisibility = event;
    }
  
    toggleUpdateEvalutionModal(): void {
      this.updateEvaluationModalVisibility = !this.updateEvaluationModalVisibility;
    }
